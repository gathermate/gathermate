...
$(window).scroll(function(){
    if ($(window).scrollTop() == $(document).height() - $(window).height()){
        if (hasNext){
            getChannels(streamer, currentPage + 1);
        }
    }
});
function getChannels(streamer, page){
    var url = streamer + '/channels'
...
<img class="lazy w-100" src="' + streamer + '/resource?url=' + encodeURIComponent(item.thumbnail) + '" class="card-img-top"> \
<div class="card-body p-2"> \
<h5 class="card-title"><a href="javascript:void(0);" onclick="playChannel(\'' + streamer + '\', \''+ item.id + '\')">' + item.name + '</a></h5> \
<p class="card-text">' + item.cProgram + '</p> \
</div> \
</div>';
        ...
        console.log('Failed .getJSON : ' + url);
    }).always(function(){
        //console.log('always.');
    });
}
